...
for gid in GeneNukes.iterkeys():  	
	...
	ATexpcount = ATEXP * float(GeneNukes[gid]['length'])
	GeneNukes[gid]['ATCountexp'] = ATexpcount
	
	CEXP = (GeneNukes[gid]['C'])/float(GeneNukes[gid]['length'])
	GEXP = (GeneNukes[gid]['G'])/float(GeneNukes[gid]['length'])
	CGEXP = CEXP * float(GEXP)
	CGexpcount = CGEXP * float(GeneNukes[gid]['length'])			   
	GeneNukes[gid]['CGCountexp'] = CGexpcount		   
				   
	#put expected AT frequency into dict
	GeneNukes[gid]['ATEXP'] = ATEXP
	GeneNukes[gid]['CGEXP'] = CGEXP		   
...
OUT = open("77-Rhoades.Final.OUTPUT.txt", 'w')
OUT.write("Gene\tATfrac\tGCfrac\tATobs\tCGobs\tATexp\tCGexp\n")
for gid in GeneNukes.iterkeys():
	OUT.write("%s\t%0.2f\t%0.2f\t%0.1f\t%0.1f\t%0.4f\t%0.4f\n" % (gid, GeneNukes[gid]['A+T'], GeneNukes[gid]['C+G'], GeneNukes[gid]['ATCount'],GeneNukes[gid]['CGCount'], GeneNukes[gid]['ATCountexp'], GeneNukes[gid]['CGCountexp']  ) )
